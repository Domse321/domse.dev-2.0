...
test('Sportseite ist eigenständig, semantisch und strikt abgesichert', () => {
  ...
  assert.match(html, /class="skip-link" href="#inhalt"/);
...
test('Sessionzustand ist datumsbezogen, legacy-kompatibel und storage-resilient', () => {
  ...
  assert.match(js, /domse-sport-legacy-migrated-v2/);
  ...
  assert.match(js, /maxSessionMs/);
  assert.match(js, /ensureCurrentSession/);
...
test('Bedienung und Layout berücksichtigen Tastatur, Touch und reduzierte Bewegung', () => {
  ...
  assert.match(html, /aria-labelledby="progressLabel"/);
  assert.match(html, /id="sessionStatus" aria-live="polite"/);
